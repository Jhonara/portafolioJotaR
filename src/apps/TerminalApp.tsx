...
import { BadgeCheck, Coffee, IdCard, PartyPopper, Rocket } from "lucide-react";
...
const responses: Record<string, string> = {
  ...
  pokemon: "Liga JotaR localizada. Abriendo perfil de campeón…", matrix: "Wake up, Jhonatan. The portfolio has you.",
  night: "Night Shift activado. Señal reducida, creatividad aumentada.", neon: "Neon Core restaurado. Todos los canales de energía online.",
  "sudo hire jhonatan": "Solicitud elevada al comité de decisiones. Excelente decisión.",
  increase: "Análisis de compensación abierto en una nueva ventana.",
};
...
const playTone = (frequency: number, duration = 0.12) => {
  ...
  gain.gain.setValueAtTime(0.035, context.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, context.currentTime + duration);
...
const TerminalApp = () => {
  ...
  useEffect(() => { if (!effect) return; const timer = window.setTimeout(() => setEffect(null), 3200); return () => window.clearTimeout(timer); }, [effect]);
  const runCommand = (event: FormEvent) => {
    event.preventDefault(); const command = input.trim().toLowerCase(); if (!command) return; setInput("");
    if (command === "clear") { setOutput([]); setEffect(null); return; }
    if (command === "help") { setOutput((lines) => [...lines, help]); return; }
    const titles: Record<string, string> = { about: "About", projects: "Projects", pokemon: "Pokémon League", increase: "Análisis de Compensación" };
    if (command in titles) { openWindow({ id: command, title: titles[command] }); setOutput((lines) => [...lines, `Opening ${command}.app...`]); playTone(520); return; }
    const nextEffect: TerminalEffect = command === "sudo hire jhonatan" ? "hire" : command === "whoami" ? "whoami" : ["matrix", "night", "neon", "java", "coffee"].includes(command) ? command as TerminalEffect : null;
    if (nextEffect) setEffect(nextEffect);
    if (command === "matrix") { setTheme("matrix"); playTone(180, 0.24); } if (command === "night") { setTheme("night"); playTone(260, 0.18); } if (command === "neon") { setTheme("neon"); playTone(720, 0.14); } if (command === "java") playTone(430, 0.12); if (command === "coffee") { activateCoffee(); playTone(340, 0.18); } if (command === "sudo hire jhonatan") playTone(620, 0.25);
