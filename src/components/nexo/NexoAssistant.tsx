...
const NexoAssistant = () => {
  ...
  const send = async (question: string) => { const clean = question.trim(); if (!clean || isTyping) return; const userMessage = { id: crypto.randomUUID(), role: "user" as const, text: clean }; const nextMessages = [...messages, userMessage].slice(-20); setMessages(nextMessages); setInput(""); setIsTyping(true); setMood("typing"); const known = getPortfolioAnswer(clean, language, messages); try { const aiAnswer = known ? null : await askNexoAI(nextMessages, language); const result = known ?? (aiAnswer ? { answer: aiAnswer, mood: "happy" as NexoMood } : getFallbackAnswer(clean, language, messages)); if (!known && !aiAnswer) await new Promise((resolve) => window.setTimeout(resolve, 520)); if (result.open) showWindow(result.open); setMood(result.mood); setBubble(result.answer); setMessages((current) => [...current, { id: crypto.randomUUID(), role: "nexo" as const, text: result.answer }].slice(-20)); } catch { const result = getFallbackAnswer(clean, language, messages); setMood(result.mood); setBubble(result.answer); setMessages((current) => [...current, { id: crypto.randomUUID(), role: "nexo" as const, text: result.answer }].slice(-20)); } finally { setIsTyping(false); } };
