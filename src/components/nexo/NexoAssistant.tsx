...
const NexoAssistant = () => {
  ...
  const [lookAt, setLookAt] = useState({ x: 0, y: 0 }); const lastAmbient = useRef(-1);
  ...
  useEffect(() => { const track = (event: MouseEvent) => setLookAt({ x: Math.max(-3, Math.min(3, (event.clientX / window.innerWidth - .5) * 6)), y: Math.max(-2, Math.min(2, (event.clientY / window.innerHeight - .5) * 4)) }); window.addEventListener("mousemove", track, { passive: true }); return () => window.removeEventListener("mousemove", track); }, []);
  ...
  useEffect(() => { if (open || isTyping) return; const timer = window.setInterval(() => { const actions: NexoMood[] = ["idle", "happy", "thinking", "waving", "sleepy"]; setMood(actions[Math.floor(Math.random() * actions.length)]); }, 7000); return () => window.clearInterval(timer); }, [open, isTyping]);

  const showWindow = (id: NonNullable<ReturnType<typeof getFallbackAnswer>["open"]>) => { const labels = { about: language === "es" ? "Sobre mí" : "About me", projects: language === "es" ? "Proyectos" : "Projects", skills: language === "es" ? "Habilidades" : "Skills", experience: language === "es" ? "Experiencia" : "Experience", terminal: "Terminal" }; openWindow({ id, title: labels[id] }); };
  const send = async (question: string) => { const clean = question.trim(); if (!clean || isTyping) return; const userMessage = { id: crypto.randomUUID(), role: "user" as const, text: clean }; const nextMessages = [...messages, userMessage]; setMessages(nextMessages); setInput(""); setIsTyping(true); setMood("typing"); const known = getPortfolioAnswer(clean, language); try { const aiAnswer = known ? null : await askNexoAI(nextMessages, language); const result = known ?? (aiAnswer ? { answer: aiAnswer, mood: "happy" as NexoMood } : getFallbackAnswer(clean, language)); if (!known && !aiAnswer) await new Promise((resolve) => window.setTimeout(resolve, 520)); if (result.open) showWindow(result.open); setMood(result.mood); setBubble(result.answer); setMessages((current) => [...current, { id: crypto.randomUUID(), role: "nexo", text: result.answer }]); } catch { const result = getFallbackAnswer(clean, language); setMood(result.mood); setBubble(result.answer); setMessages((current) => [...current, { id: crypto.randomUUID(), role: "nexo", text: result.answer }]); } finally { setIsTyping(false); } };
